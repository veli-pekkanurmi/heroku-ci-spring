...
public class EmployeeControllerTest {
    ...
    public void whenCreateGetEmployee_thenOk() throws Exception {
        ...
        this.mockMvc.perform(get("/employees")).andExpect(status().isOk()).andExpect(content().contentType(CONTENT_TYPE)).andExpect(jsonPath("$", hasSize(4))).andExpect(jsonPath("$[0].name", is("Mikko"))).andExpect(jsonPath("$[3].name", is("john")));
    }
}
